...
class ReportsController extends AppController {
  ...
  function edit($id = null) {
    ...
    if (!$id && empty($this->request->data)) {
      $this->Session->setFlash(__('Неверный ID для заключения.'), 'jgrowl');
    ...
    if (!empty($this->request->data)) {
      ...
      if ($this->Report->save($this->request->data)) {
        $savecontrol1 = true;
        if ($this->request->data['subraport']) {
          $tempcount = 1;
          $multipleedit = $this->request->data['Project']['reportmultipleedit'];
          ...
          foreach ($this->request->data['subraport'] as $tempsubraportkey => $tempsubraportvalue) {
            $this->request->data['subraport'][$tempsubraportkey]['Subreport']['report_id'] = $this->Report->id;
            if ($this->request->data['subraport'][$tempsubraportkey]['Subreport']['expert_id'] == '')
              $this->request->data['subraport'][$tempsubraportkey]['Subreport']['expert_id'] = $loginedexpertid;
            $this->request->data['subraport'][$tempsubraportkey]['Subreport']['text'] = str_replace(chr(194).chr(160), ' ', $this->request->data['subraport'][$tempsubraportkey]['Subreport']['text']);
            $this->request->data['subraport'][$tempsubraportkey]['Subreport']['obiectia'] = str_replace(chr(194).chr(160), ' ', $this->request->data['subraport'][$tempsubraportkey]['Subreport']['obiectia']);
            if ($multipleedit == 0 || $isadmin == 1) {
              if ($this->request->data['subraport'][$tempsubraportkey]['Subreport']['todelete'] != 1) {
                $this->request->data['subraport'][$tempsubraportkey]['Subreport']['subreportorder'] = $tempcount;
                $tempcount++;
              }
            }
            else {
              if ($this->request->data['subraport'][$tempsubraportkey]['Subreport']['todelete'] != 1) {
                $this->request->data['subraport'][$tempsubraportkey]['Subreport']['subreportorder'] = $tempcountarray[$this->request->data['subraport'][$tempsubraportkey]['Subreport']['expert_id']];
                $tempcountarray[$this->request->data['subraport'][$tempsubraportkey]['Subreport']['expert_id']]++;
              }
            }
          }
          if ($this->Report->Subreport->saveAll($this->request->data['subraport'])) {
            $savecontrol2 = true;
        ...
        if ($this->request->data['Attachment']) {
          foreach ($this->request->data['Attachment'] as $attachmentkey => $attachmentvalue) {
            $this->request->data['Attachment'][$attachmentkey]['report_id'] = $this->Report->id;
            if ($attachmentvalue['attachmentfile']['error'] == 0) {
              $this->request->data['Attachment'][$attachmentkey]['filename'] = $attachmentvalue['attachmentfile']['name'];
              move_uploaded_file($attachmentvalue['attachmentfile']['tmp_name'], 'uploaded/annexes/'.$attachmentvalue['attachmentfile']['name']);
            }
            else if (!isset($this->request->data['Attachment'][$attachmentkey]['filename']))
              $this->request->data['Attachment'][$attachmentkey]['filename'] = '';
          }
          $this->Report->Attachment->saveAll($this->request->data['Attachment']);
          $this->Report->Attachment->deleteAll(array('Attachment.todelete'=>1));
        }
      }
      if ($savecontrol1 && $savecontrol2) {
        $this->Session->setFlash(__('Заключение было сохранено.'), 'jgrowl');
        $this->redirect($backlink);
      }
      else $this->Session->setFlash(__('Заключение не может быть сохранено. Проверьте введенные данные и попробуйте еще раз.'), 'jgrowl');
    }
    if (empty($this->request->data)) {
      $this->Report->recursive = 1;
      $this->Report->Subreport->recursive = 1;
      $this->Report->Attachment->recursive = 0;
      $this->Report->Project->Author->recursive = -1;
      $this->request->data = $this->Report->read(null, $id);
      $multipleedit = $this->request->data['Project']['reportmultipleedit'];
      $conditions = array('Subreport.report_id'=>$id);
      if ($isadmin == 0 && $multipleedit == 1 && $loginedexpertid != $this->request->data['Project']['expert_id'])
      ...
      $this->request->data['subraport'] = $subreports;
      unset($this->request->data['Project']['Report']);
      unset($this->request->data['Subreport']);
    }
    if (($this->request->data['Project']['projectstate'] == 2) && ($isadmin == 1)) $celemsacceptance = 1;
    ...
    if ($multipleedit == 0 || ($multipleedit == 1 && $isadmin == 1) || ($multipleedit == 1 && $loginedexpertid == $this->request->data['Project']['expert_id']))
  ...
  function view($id = null, $type = null) {
    ...
    $this->Report->recursive = 1;
    $this->Report->Subreport->recursive = 1;
    $this->Report->Attachment->recursive = -1;
    $this->Report->Project->Author->recursive = -1;
    $this->Report->Subreport->Celem->recursive = -1;
    $this->request->data = $this->Report->read(null, $id);
    ...
    foreach ($this->request->data['Attachment'] as $attachmentkey => $attachmentvalue) {
      unset($this->request->data['Attachment'][$attachmentkey]['Report']);
    }
    unset($this->request->data['Project']['Report']);
    unset($this->request->data['Subreport']);
    $author = $this->Report->Project->Author->find('list');
  ...
  function admin_statistic() {
    ...
    $filter = array();
    if ($this->request->data['Project']['expert_id'] != '')
      $filter = array_merge($filter, array('Project.expert_id'=>$this->request->data['Project']['expert_id']));
    if ($this->request->data['Project']['author_id'] != '')
      $filter = array_merge($filter, array('Project.author_id'=>$this->request->data['Project']['author_id']));
    if ($this->request->data['Project']['projecttype'] != '')
      $filter = array_merge($filter, array('Project.projecttype'=>$this->request->data['Project']['projecttype']));
    ...
    if ($this->request->data['Report']['date2'] != '') {
      $filterreports = array_merge($filterreports, array('Report.reportdate <= '=>$this->request->data['Report']['date2']));
    }
